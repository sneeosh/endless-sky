# Endless Sky — Story Ideas (riff space)

Snapshot of the upstream [StoryIdeas wiki page](https://github.com/endless-sky/endless-sky/wiki/StoryIdeas)
as of wiki commit `0c0ba70` (2026-05-06). Pulled in here so we can extend and
riff on it without round-tripping the wiki. Anything below the
`## Riffs / extensions` divider is local — original wiki content stays above
the line so future syncs are easy.

---

## Upstream wiki content
...
Working area for our own additions, candidate picks, and outlines. Anything
below this line is local to this repo, not from the upstream wiki.

### Candidate pick: Sheragi artifact dig

Pulled from the **Alien artifacts** section. The hook is that the Unfettered
Hai explicitly say human space was occupied by the Sheragi and the Hai before
humans arrived, and the wiki asks: are there artifacts left, did the Drak or
Quarg sweep the planets clean, and are the Sheragi actually dead?

Sketch (placeholder — riff freely below):

- **Inciting incident.** A geological / archaeological find on a human
  frontier world turns up something obviously non-human and obviously
  *very* old. Player is hired to ferry the find, an expert, or both.
- **The dig escalates.** As more is uncovered, the Quarg take an
  unusually pointed interest. Conversations with Hai (Unfettered vs.
  mainstream) give conflicting accounts. Possibility for a Remnant
  scholar cameo, since they hoard pre-human tech.
- **The dilemma.** Hand the find to the Republic / Syndicate / a Hai
  faction / the Quarg / keep it. Each path has costs. Sheragi
  themselves stay off-screen — preserving the wiki's "lies and
  half-truths" principle.
- **What it must not break.** No claim that the Sheragi are alive (or
  dead). No new playable faction. No conflict with Patir Mystery,
  Remnant, or Hai Reveal arcs — this is a short side plot, not a
  campaign.

Open questions / things to riff on:

- Which human system hosts the dig? Needs a frontier-ish world with
  little existing lore baggage.
- Quarg response: warning, sabotage, or just unnervingly polite
  observation?
- Reward shape: a unique outfit, a snippet of lore, a permanent
  reputation shift, or just credits + a story beat?
